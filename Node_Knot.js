...
var LinesToMove = []
var freshPuzzle = []
var DragPoints = [[null,null],[null,null]]
var Dist = 0
var Time = 0
var Initials = ''
var StartTime = null
...
function mainLoop(){
	...
	if(InputFreeze == false && ResultsDisplayed == false && WelcomeClosed == true){
	    ...
	    if(Mode == 'create'){
	        ...
	        if(document.getElementById('createinstruction1').style.visibility == 'visible'){
                if(LineArray.length > 2){
                    document.getElementById('createinstruction1').style.visibility = 'hidden'
                }
	        }
    		switch(MouseState){
    			case 'moving':
...
function createNewLine(){
	...
	var lineLength = Math.sqrt(Math.pow(previewLine.startx-lineendx,2) + Math.pow(previewLine.starty-lineendy,2))
	if (lineLength > 5){
	    //if it's a purposefully created line, add it to the LineArray!
		LineArray.push(new Line(previewLine.startx, previewLine.starty, lineendx, lineendy, ElementID))
	}
...
function isOnEndpoint(xin,yin,useAux){
	...
	for(var i = 0; i < LineArray.length; i ++){
		...
		if (Math.pow(Math.pow(l.startx - x,2)+ Math.pow(l.starty - y,2),.5) < endpointSnapDist){
			var sx = l.startx
			var sy = l.starty
		...
		if (Math.pow(Math.pow(l.endx - x,2)+ Math.pow(l.endy - y,2),.5) < endpointSnapDist){
			var ex = l.endx
			var ey = l.endy
...
function getEPs(epToMove){
    ...
    for(var i = 0; i < LineArray.length; i++){
        ...
        else if((LineArray[i].startx == epToMove[1]) && (LineArray[i].starty == epToMove[2])){
...
function moveEPs(ep,wasx,wasy){
	...
	Dist = (Dist + Math.sqrt(Math.pow(wasx-Mouse.x,2) + Math.pow(wasy-Mouse.y,2)))
...
function intersections(){
	...
	for(var i = 0; i < LineArray.length; i++){
		...
		for(var j = 0; j < LineArray.length; j++){
            ...
            if(closeness1 < .01 && closeness2 < .01 && isOnEndpoint(x,y, true)[0] == false){
				redraw()
				return true
			}
		}
...
function success(){
	...
	document.getElementById('resultstxt').value = 'Time: ' + totalTime + '\nDist: ' + totalDist.toFixed(1)
...
function rankings(time, dist){
	...
	document.getElementById('fastesttxt').value = 'Fastest:' + '\n   1) ' + top3Fastest[0][0] + ': ' + top3Fastest[0][1] + '\n   2) ' + top3Fastest[1][0] + ': ' + top3Fastest[1][1] +
	'\n   3) ' + top3Fastest[2][0] + ': ' + top3Fastest[2][1]

	document.getElementById('shortesttxt').value = 'Shortest: ' + '\n   1) ' + top3Shortest[0][0] + ': ' + top3Shortest[0][1] + '\n   2) ' + top3Shortest[1][0] + ': ' + top3Shortest[1][1] +
	'\n   3) ' + top3Shortest[2][0] + ': ' + top3Shortest[2][1]
...
function submitbtn(){
    ...
    if(closed() == false){
        alert('Shape must be closed before it can be sumitted. All nodes must connect an even number of lines.')
    }
    else if(intersections() == false){
        alert('This shape has no overlapping lines. Pretty boring puzzle. Cannot be subimtted.')
    }
    else{
...
function sendToDatabase(){
    ...
    sendArray = ['0', Initials, JSON.stringify(LineArray), '???' , '---','???','---','???','---','???','---','???','---','???','---']
	...
	ourRequest.onload = function(){
	    ...
	    //If there is a communication issue with the server, display an alert.
	    if(ourRequest.status != 200){
	        alert('It looks like communication with the server is borked. Could not save your shape. :(')
	        return -1
	    }
...
function updateScoresInDatabase(){
    ...
    sendArray = [CurrentID, top3Fastest[0][0] , top3Fastest[0][1] , top3Fastest[1][0], top3Fastest[1][1], top3Fastest[2][0], top3Fastest[2][1], top3Shortest[0][0] , top3Shortest[0][1] , top3Shortest[1][0], top3Shortest[1][1], top3Shortest[2][0], top3Shortest[2][1]]

	ourRequest.open('POST', 'https://derickthomas.pythonanywhere.com/updateScores');
	ourRequest.setRequestHeader("Content-type", "text/plain");
	ourRequest.send(sendArray);
	...
	ourRequest.onload = function(){
	    if(ourRequest.status != 200){
	        alert('It looks like communication with the server got messed up - your score was not saved :(')
	    }
        return 0
	}
